...
async function setup() {
  await runner({
    databaseUrl: process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/dhis2_style_c",
  ...
  console.log("Source C (DHIS2-style) database migrated and seeded.");
  await pool.end();
}

setup().catch((err) => {
  console.error("Source C DB setup failed:", err);
  process.exitCode = 1;
});
