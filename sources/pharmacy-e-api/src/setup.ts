...
async function setup() {
  await runner({
    databaseUrl: process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/pharmacy_e",
  ...
  console.log("Pharmacy E database migrated and seeded.");
  await pool.end();
}

setup().catch((err) => {
  console.error("Pharmacy E DB setup failed:", err);
  process.exitCode = 1;
});
