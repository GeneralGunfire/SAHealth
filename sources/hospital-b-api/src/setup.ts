...
async function setup() {
  await runner({
    databaseUrl: process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/hospital_b",
  ...
  console.log("Hospital B database migrated and seeded.");
  await pool.end();
}

setup().catch((err) => {
  console.error("Hospital B DB setup failed:", err);
  process.exitCode = 1;
});
